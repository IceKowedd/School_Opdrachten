...
public class SkoolClasses {
  ...
  public static void addToClass(Scanner scanner) {
    ...
    for (SkoolClasses skoolClass : allClasses) { logger.info("- " + skoolClass.getName()); }
    ...
    } else { logger.info("Invalid class name. Please enter a valid class name from the list."); } }

  public static void printClasses() {
    Logger logger = Logger.getLogger(Opdracht_4.class.getName());
    if (allClasses.isEmpty()) {
      logger.info("No classes available."); } else { logger.info("List of Classes:");
      for (SkoolClasses skoolClass : allClasses) { logger.info(skoolClass.getName());
        logger.info("Students:");
        for (Student student : skoolClass.getClassStudents()) { logger.info("- " + student.getName()); } // print students of class.
        logger.info("Counsellor:"); ArrayList<Counsellor> classCounsellor = skoolClass.getClassCounsellor();
        if (!classCounsellor.isEmpty()) { logger.info("- " + classCounsellor.getFirst().getName()); // print the first counsellor that has been appointed. (if 2 or more accidentally have been appointed)
        } else { logger.info("- None"); } logger.info("\n"); } } } // Print None of no counsellors have been appointed.
  ...
  public static void addNewClass(Scanner scanner) {
    ...
    allClasses.add(newClass); logger.info(message);
  ...
  private static int getStudentIndex(boolean isValidInput, String s, int studentIndex, Scanner scanner) {
    Logger logger = Logger.getLogger(Opdracht_4.class.getName());
    while (!isValidInput) {
      logger.info(s);
      try {
  ...
  public static void addCounsellor2(Scanner scanner, SkoolClasses selectedClass) {
    ...
    } else { logger.info("Invalid counsellor number. No counsellor added to the class."); } }
  ...
  public static void removeStudentFromClass(Scanner scanner, SkoolClasses selectedClass) {
    ...
    while (removingStudents) {
      logger.info("Select students to remove from the class (Enter student number or 0 to finish):");
      ...
      } else { logger.info("Invalid student number. Please try again."); } } }
  ...
  public static void removeCounsellorFromClass(Scanner scanner, SkoolClasses selectedClass) {
    ...
    if (classCounsellors.isEmpty()) {
      logger.info("There are no counsellors assigned to this class."); return;}
    ...
    } else { logger.info("Invalid counsellor number. No counsellor removed from the class."); } }
  ...
  public static void deleteClass(Scanner scanner) {
    ...
    if (targetClass != null) { allClasses.remove(targetClass);
      String message = String.format("Class '%s' has been deleted", className); logger.info(message);
    } else { String message = String.format("Class '%s' not found.", className); logger.info(message); } }
